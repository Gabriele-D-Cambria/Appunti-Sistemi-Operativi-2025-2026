...
Il compoto del sottosistema di I/O è quello di **_nascondere i dettagli hardware dei controllori dei dispositivi_**, garantendo una gestione omogenea dei diversi dispositivi.
...
- **Controllore**: è dell'hardware _special purpose_ che ha il duplice scopo di gestire la periferica tramite un bus dedicato, e comunicare con il sistema attraverso il _bus_ condiviso.

<img class="" src="./images/Gestione Periferiche/general-scheme.png">


Il controllore è a tutti gli effetti un dispositivo programmabile, anche se questo avviene molto raramente. Infatti il controllore esegue tendnezialmente del _firmware_ interno, che gli permette di eseguire delle azioni che possiamo a tutti gli effetti classificare come **processi esterni**.
...
Il sotto sistema di I/O deve anche:
...
Questa interfaccia comunica con dell'altro _software_ che stavolta è _device-dependent_ dividendo le periferiche in diversi categorie:
...
Un'altra delle funzioni più importanti offerte dal livello software indipendente dai dispositivi è quello di fornire una o più aree di memroia tampone, dette `buffer` destinate a contenere i dati durante il trasferimento tra il dispositivo e l'area di memoria virtuale del processo applicativo da cui i dati devono essere prelevati/inseriti.

Ci sono diverse ragioni per le quali riserviamo questi _buffer_, prima fra tutte quella di far fronte alla notevole differenza fra la velocità con cui il processore può produrre/consumare informazioni e quella del dispositico di consumarle/produrle.

L'introduzione dei _buffer_ introduce anche dell'_overhead_ dovuto alla doppia copia dei dati, prima dal dispositivo al _buffer_ e dopo dal _buffer_ all'`u-buf` del processo applicativo.
...
* @param ubuf indirizzo del buffer nela memoria virtuale del processo
* @param nbytes numero di byte richiesti
*/
n = read(fd, ubuf, nbytes);
```

Normalmente ul processo applicativo deve sospendere la propria esecuzione fino a quando i dati richiesti sono stati scritti in `ubuf`.
Se la `read` è implementata facendo ricorso a un buffer di sistema l'operazione di lettura tarsferisce i dati _di un intero settore_ prima dal disco al biffer, e quindi da questo all'area `ubuf` del processo utente solo per la dimensione `nbytes`, cancellando in maniera del tutto trasparente caratteri letti ma successivamente cancellati.

Il vantaggio di questa organizzazione si rileva quando il processo deve leggere ed eleaborare _sequenze di blocchi_, poiché il tempo di attesa del processo e il numero di commutazioni di contesto può essere ridotto se l'elaborazoine di un blocco di dati vien eeseguita **in parallelo alla lettura nel buffer del successivo blocco di dati**, in una politica di _read ahead_, in particolare se, come nell'immagine sopra, sono presenti più _buffer_ di sistema in un sistema circolare da poter utilizzare.
...
Nel caso il dispositivo possa svolgere più operazioni, ha senso immaginare alla presenza di altri bit che permettono di fae la selezione sul comportamento che il dispositivo deve assumere.
È poi sicuramente presente un bit di _abilitazione alle interruzioni_ che abilita il controllare a inviare segnali di interruzioni alla **CPU** per segnalare la temrinazione dell'operazione del dispositivo.
...
La differenza però è che il processo non corrisponde all'eseuzione di un programma, ma piuttosto ad una **sequenza di azioni cablate svolte da un dispositivo che opera in parallelo alla CPU**.
Per distiguenrlo lo identificheremo con il termine di **_processo esterno_**.
...
Per forzare la sospensione del processo quanda deve restare in attesa sul flag è possibile **associare al dispositivo un semaforo inizializzato a zero** e sostituire un ciclo di attesa attiva una `wait` su tale semaforo, modificando il frammento di codice:
```cpp
...
Il processo adesso si sospende, quindi non siamo più in grado di controllare direttamente la disponibilità del dato nel registro del controllore.
Dovrà quindi essere il controllore stesso a segnalare l'evento risvegliando il processo applicativo, utilizzando una funzione che abbia delegato tale compito.
...
Indichiamo:
- `PI`: il processo applicativo che esegue l'operazione di input degli `n` dati
- `PE`: il processo esterno
...
Nell'esempio appena fatto `PI` trasferisce un solo byte alla volta, tuttavia questo è atamente inefficiente.
Se fosse necessario trasferire $n$ byte, avremmo infatti un processo che per $n$ volte viene schedulato solo per poi tornare in attesa.
...
In questo caso `PI` carica la locazione di memoria dove si drovano i byte, alla quale `inth` accederà.
...
Per ottenere il comportamento descritto iin'ora è necessario che la routine di interruzione riesca a discriminare quali sono le interruzioni intermedie di ogni blocco di trasferimenti e qual è quella finale in corrispondenza della quale deve essere riattivato il processo applicativo.

Un modo per fare ciò è riservare un memoria una **struttura dati destinata a rappresentare il dispositivo**, chiamata _descrittore del dispositivo_, sulla quale possa operare:
- **Il processo applicativo**; attraversi le funzioni offerte dall'interfaccia _device independent_
- **La routine di gestione delle interruzioni** lanciate dal dispositivo.

L'unione tra _descrittore del dispositivo_ e funzioni di accesso, costituiamo il **_device driver_**.

</div>
<div class="">
<img class="" src="./images/Gestione Periferiche/device-driver.png">
</div>
</div>
...
- consentire la comunicazioni di informazioni tra il processo applicativo e il dispositivo e viceversa.
...
int read(int disp, char* pbuf, int cont){
	// descrtittore è la lista dei descrittori indicizzadi tramite il numero dispositivo
...
All'arrivo di un'interruzione il processo in esecuzione viene interrotto, e via hardaware, va in esecuzione la funzione `inth`m che leggerà per prima cosa il registro di stato per verificare la causa dell'interruzione.
Se questa non è dovuta ad errori il legge il dato prelevandolo dal registro dati e trasferendolo nel buffer di sistema all'indirizzo contenuto in `puntatore`.
...
Al fine di chiarire quanto detto finora, illustriamo com'è trutturato il flusso di controllo durante l'esecuzione di una chamata di sistema relativa ad un trasferimento di dati.

<div class="grid2">
<div class="">

Nell'immagine sulla destra troviamo tre sezioni principali:
- Quella sulla sinistra è l'equivalente del `IO API`
- Quella in alto sulla destra rappresenta il codice del sistema operativo che è **_device independent_**
- Quella in basso sulla destra invece è l'implementazione **device dependent**, con la funzione `_read` che è tipicamente preceduta dal carattere `_`.

</div>
<div class="">
<img class="" src="./images/Gestione Periferiche/flux-control.png">
</div>
</div>

### 2.2.2. Gestione del temporizzatore

Un dispositivo molto particolare è costituito dal _timer_.

La sua particolarità risiede nel fatto che viene utilizzato come una **_fonte di interruzione cadenzate nel tempo_**.
...
Ciascun semaforo viene utilizzato epr bloccare il corrispondente processo che invocca la primitiva `delay`.
...
Il numero di bit memorizzabili all'interno di ogni traccia è lo steso per tutte le tracce, così da semplificare la logica di controllo del dispositivo.
...
In unmero di settori all'interno di una stessa traccia è dell'ordine di alcune centinaia.

Nelal tabella di seguito vediamo i dati di alcuni dischi Western Digital
...
| **minimo di _seek_ tra cilindri adiacenti** | Tempo minimo necessario alla testina per spostarsi sulla traccia corretta |   $4ms$    |  $0.6ms$   |
|          **Tempo media di _seek_**          | Tempo medio necessario alla testina per spostarsi sulla traccia corretta  |   $11ms$   |  $5.2ms$   |
|           **Tempo di rotazione**            | Tempo necessari oal disco per effettuare una rotazione completa           |  $13 ms$   |   $6 ms$   |
...
Per i disci presenti sopra possiamo calcolare:
...
Con riferimento all'immagine precedente (immaginando che la situazione di partenza sia decrescente) le richieste sarebbero servite nell'ordine `20`, `14`, `7`, `23`, `40` e `47`, per uno spostamento totale di $53$.
