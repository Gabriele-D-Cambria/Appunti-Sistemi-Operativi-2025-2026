...
- [2. Filesystem](#2-filesystem)
	- [2.1. Filesystem in Unix/Linux](#21-filesystem-in-unixlinux)
...
Tutti i disci e le relative partizioni vengono rese accessibili mediante un unico _filesystem virtuale_.

L'atto di rendere accessibile una partizione si dice **_montaggio_**.

<img class="" src="./images/filesystem-tree.png">


All'interno del _virtual filesystem_ vi sono alcune cartelle principali:
...
## 2.1. Filesystem in Unix/Linux

I sistemi `Unix` sono caratterizzato dall'omogeneità, in particolare ogni risorsa del sistema è rappresentata come un **file**.
...
In particolare i **directory file** sono costituiti da una serie di _record_ che descrivono il contenuto della directory:
```file
...
Quando viene creato un file di nome `fileName` viene creato un nuovo `i-node` e viene aggiunto alla `i-List`, identificandolo con un `i-number IN`
Al contenuto della directory viene aggiunto un nuovo record `<fileName, IN>`.

Dopo la creazione, il file ha **un solo nome**, che combacia con l'_hard link_ al descrittore del file.

Il _filesistem_ permette di **definire più _hard link_ associato ad un `i-node`**, generando a tutti gli effetti nomi alternativi per lo stesso file.

È possibile inoltre definire anche collegamenti simbolici (_soft-link_), per avere **nomi alternativo per fare riferimento ad un hard link**.

<div class="flexbox" markdown="1">

|                                             |                            Hard Link                            |                       Soft Link                       |
| :-----------------------------------------: | :-------------------------------------------------------------: | :---------------------------------------------------: |
| Eliminazione o spostamento dell'_hard link_ |          Non ha alcun effetto sugli altri _hard link_           | Il _soft link_ non è più in grado di accedere al file |
|                   Effetti                   | L'`i-node` viene eliminato solo se il numero di _hard link_ è 0 |         Non ha alcun effetto sull'_hard link_         |
...
<img class="" src="./images/hard-and-soft-links.png">
...
All'interno del descrittore di processo (_user structure_) si trova una **tabella dei file aperti di processo**, dove ogni elemento, detto _file descriptor_, è un riferimento all'elemento corrispondente nella **Tabella di File Aperti di Sistema**.

All'interno della **Tabella dei File Aperti di Sistema** si trovano tanti elementi quanti sono i file aperti dal sistema.
Se due processi diversi aprono lo stesso file avremo **_due entry separate_**.
All'interno dei record troviamo un `I/O pointer` al file e un riferimento all'`i-node` del file.
...
<div class="">
<img class="" src="./images/file-access-structs.png">
</div>
</div>

Quando viene generato un processo figlio, questo erediterà una copia della `User Structure`, quindi anche **una copia del file descriptor**.

In questo caso i due processi hanno descrittori che **_puntano allo stesso elemento della Tabella di File di Sistema_** e quindi **_condividono l'I/O pointer nell'accesso sequenziale al file_**.

<img class="" src="./images/file-access-structs-fork.png">


Per l'accesso ai file sono presennti diverse primitive:
```c
/**
...
* @param fd file descriptor che si vuole chiudere. È geenrato da funzioni come open(), pipe(), ...
...
È un tipo di _comunicazione indiretta_ senza naming esplicitp, reailzzando il concetto di _mailbox_ nella quale si possono accordare messaaggi in modo `FIFO`, in buffer di dimensioni predefinite.

La `pipe` è un canale monodirezionale a due estremi (uno per la lettura e uno per la scrittura) che hanno associati un `file descriptor`.
...
I processi figli **_ereditano una copia degli stessi file descriptor_**. Questi possono utilizzarli per comunicare con il padre e gli altri figli.

Nel caso di comunicazioni di processi in gerarchie diverse si utilizzano altri strumenti, come i socket.

Il comportamento di `read` su estremità della pipe **_è bloccante se il buffer è vuoto e c'è almento uno scrittore attivo_**.
Restituisce inoltre `EOF` (ovvero `0`) se il buffer è vuoto e tutti gli scrittori hanno chiuso il _file descriptor_.

Il comportamnto di write sulla estremità **_è bloccante se il buffer è pieno_**, e restituisce **_errore_** se non ci sono lettori attivi.

Di conseguenza diventa importante **_Chiudere gli estremi non usati_**. Ovvero `pipefd[1]` per i lettori e `pipefd[0]` per gli scrittori.

In questo modo possiamo evitare _deadlock_ e comportamenti inattesi.

# 3. Pilotare Applicazioni

Abbiamo già visto e utilizzato `STDIN`, `STDOUT` e `STDERR` che sono descrittori di _defaults_. Sono generati automaticamente al momento dell'esecuzione del programma, e la loro apertura e chiusura è gestita _automaticamente al sistema operativo_.
...
La funzione che si può utilizzare per **_duplicare un file descriptor_** è la funzione `dup2`:
```c
/**
...
È possibile combinare le funzioni `pipe` e `dup2` per redirigere il flusso dei dati dagli standard verso gli altri `fd`.
...
Attraverso la redirezione possiamo prendere applicazioni che permettono l'input attraverso `STDIN` e output verso `STDOUT` e pilotare questa applicazione tramite un programa `C` con due processi pilotando questi a nostro piacimento, anche senza conoscere il codice stesso dell'applicazione.

Come è possibile vedere dall'esempio è infatti sufficiente:
- Creare due pipe
- Eseguire una fork
- Redirigere gli `stdin` e `stdout` del figlio
- Trasformare il figlio nell'applicazione
- Far pilotare al padre il figlio seguendeo i vincoli dell'applicazione e le esigenze del programmatore
